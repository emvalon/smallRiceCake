...
static const char *TAG = "btSink";

void 
btSink_task_entry(void *arg)
{
    esp_err_t ret;
    ...
    /* Stop the codec chip */
    ret = audio_hal_ctrl_codec(board_handle->audio_hal, AUDIO_HAL_CODEC_MODE_BOTH,
                               AUDIO_HAL_CTRL_STOP);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Stop codec err: %d", ret);
    } else {
        printf("********** stop codec\n");
    }
